...
function clickViewKYC() {
    ...
    if (contractInstance.viewCustomer.call(user_name_v) == "Patient not found in Database") {
        alert("Customer not found in database!");
        return;
    }
    if (contractInstance.ifAllowed.call(user_name_v, currentEth) == false) {
        var l = confirm('Patient has not approved your pharmacy, Press OK to request permission');
        if (l == true) {
